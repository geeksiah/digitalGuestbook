...
export default function VotingPublicLayout({
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(27,212,188,0.08),transparent_28%),linear-gradient(180deg,#f9fbfa_0%,#f4f7f6_100%)] px-4 py-5 sm:px-5 sm:py-6 lg:px-6">
      <div className="mx-auto w-full max-w-7xl space-y-5">
        <div className={desktopAside ? 'space-y-5 lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] lg:items-start lg:gap-6 lg:space-y-0' : 'space-y-5'}>
          <div className="space-y-5">
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-sm leading-6 text-surface-500">{eventName}</p>
                    </div>
                  </div>
                ...
                <div className="flex flex-col gap-6 px-5 py-6 sm:px-6 sm:py-7">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    <div className="min-w-0">
                      ...
                      <h1 className="mt-2 text-3xl font-display font-bold leading-[1.06] tracking-tight text-brand-900 sm:text-[2.6rem]">
                        {eventName}
                      </h1>
                    </div>
                  </div>
            ...
            {desktopAside ? <div className="hidden space-y-5 lg:block">{children}</div> : null}
          </div>

          {desktopAside ? <div className="space-y-5">{desktopAside}</div> : null}
        </div>

        {desktopAside ? <div className="space-y-5 lg:hidden">{children}</div> : <div className="space-y-5">{children}</div>}
      </div>
    </div>
  );
}
