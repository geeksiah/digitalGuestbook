...
function normalizeBaseUrl(value: string | undefined | null): string | null {
  ...
  if (/[,\s]/.test(raw)) {
    ...
    console.warn(
      `[Site URL] Received a list of ${parts.length} values ("${raw}"). ` +
...
export function getSiteUrl(): string {
  ...
  console.warn(
    `[Site URL] FRONTEND_URL, SITE_URL or APP_URL is not set. Falling back to ${DEFAULT_SITE_URL}. ` +
...
export function pickLiveEventDomain<T extends EventDomainLike>(domains: T[] | null | undefined): T | null {
 ...
 */
export function getEventPublicBaseUrl(domains: EventDomainLike[] | null | undefined): string {
  const domain = pickLiveEventDomain(domains);
  if (!domain) return getSiteUrl();
