...
exports.mapPrismaUniquenessError = exports.PrismaVotingRepository = void 0;
const errorHandler_js_1 = require("../../../middleware/errorHandler.js");
const prisma_js_1 = __importDefault(require("../../../utils/prisma.js"));
const toSettingsString = (settings) => settings ? JSON.stringify(settings) : null;
const toConfigData = (input) => ({
...
class PrismaVotingRepository {
    db;
    constructor(db = prisma_js_1.default) {
        this.db = db;
    }
    ...
    async upsertVotingConfig(eventId, input) {
    ...
    async incrementOptionVotes(optionId, deltas) {
    ...
    async getResults(eventId, contestId) {
        ...
        const electionMap = new Map(electionVotes.map((row) => [row.optionId, Number(row._sum.voteCount || 0)]));
...
exports.PrismaVotingRepository = PrismaVotingRepository;
const mapPrismaUniquenessError = (error, fallbackMessage) => {
    const maybeError = error;
    if (maybeError?.code === 'P2002') {
        throw new errorHandler_js_1.AppError(fallbackMessage, 409);
    }
    throw error;
};
exports.mapPrismaUniquenessError = mapPrismaUniquenessError;
//# sourceMappingURL=PrismaVotingRepository.js.map