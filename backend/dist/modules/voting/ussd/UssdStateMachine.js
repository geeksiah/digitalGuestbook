...
exports.UssdStateMachine = void 0;
const PAGE_SIZE = 7;
const normalizeInput = (value) => String(value || '').trim();
const findContest = (contests, id) => contests.find((contest) => contest.id === id);
...
class UssdStateMachine {
    step(input) {
        ...
        if (input.state === 'SELECT_CONTEST') {
            ...
            if (!contest)
                return this.enterContestSelection(input.contests, 'Contest unavailable.');
        ...
        if (input.state === 'SELECT_OPTION') {
            ...
            if (!contest)
                return this.enterContestSelection(input.contests, 'Contest not found.');
            ...
            if (!option)
                return this.enterOptionSelection(contest, input.context, 'Nominee unavailable.');
            return {
                nextState: 'CONFIRM',
    ...
    enterContestSelection(contests, lead) {
    ...
    enterOptionSelection(contest, context, lead) {
...
exports.UssdStateMachine = UssdStateMachine;
//# sourceMappingURL=UssdStateMachine.js.map