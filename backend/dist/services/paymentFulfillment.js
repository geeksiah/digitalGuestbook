...
exports.fulfillVotePurchase = exports.fulfillGiftPurchase = exports.fulfillTicketPurchase = exports.releaseTicketInventoryHolds = void 0;
const errorHandler_js_1 = require("../middleware/errorHandler.js");
const prisma_js_1 = __importDefault(require("../utils/prisma.js"));
const parseMetadata = (intent) => {
    try {
        return intent.metadataJson ? JSON.parse(intent.metadataJson) : {};
    }
    catch {
        return {};
    }
};
const releaseTicketInventoryHolds = async (paymentIntentId, reason = 'RELEASED') => {
    const now = new Date();
    const status = reason === 'EXPIRED' ? 'EXPIRED' : 'RELEASED';
    await prisma_js_1.default.ticketInventoryHold.updateMany({
        where: {
            paymentIntentId,
            status: 'ACTIVE',
        },
...
exports.releaseTicketInventoryHolds = releaseTicketInventoryHolds;
const fulfillTicketPurchase = async (intent, tx) => {
    const metadata = parseMetadata(intent);
    if (!metadata.primaryName || !metadata.tickets?.length) {
        throw new errorHandler_js_1.AppError('Ticket payment intent metadata is invalid', 400);
    }
    const existing = await prisma_js_1.default.rSVP.findFirst({
        where: { paymentIntentId: intent.id },
        select: { id: true },
    });
    if (existing)
        return existing;
    await prisma_js_1.default.$transaction(async (db) => {
        ...
        if (!holds.length) {
            throw new errorHandler_js_1.AppError('No active ticket inventory hold found for payment intent', 400);
        }
        ...
        await db.rSVP.create({
            data: {
                ...
                customFields: metadata.customFields && Object.keys(metadata.customFields).length > 0
...
exports.fulfillTicketPurchase = fulfillTicketPurchase;
const fulfillGiftPurchase = async (intent, tx) => {
    const metadata = parseMetadata(intent);
    if (!metadata.guestName) {
        throw new errorHandler_js_1.AppError('Gift payment intent metadata is invalid', 400);
    }
    const existing = await prisma_js_1.default.giftOrder.findFirst({
        where: { paymentIntentId: intent.id },
        select: { id: true },
    });
    if (existing)
        return existing;
    const packageItems = (metadata.packageItems || []).filter((item) => item.giftPackageId && Number(item.quantity || 0) > 0);
    ...
    await prisma_js_1.default.$transaction(async (db) => {
        ...
        for (const item of packageItems) {
            ...
            if (!pkg)
                throw new errorHandler_js_1.AppError('Gift package not found during fulfillment', 404);
        ...
        for (const item of packageItems) {
            const pkg = packages.find((entry) => entry.id === item.giftPackageId);
            if (!pkg || pkg.stockQuantity === null || pkg.stockQuantity === undefined)
                continue;
...
exports.fulfillGiftPurchase = fulfillGiftPurchase;
const fulfillVotePurchase = async (intent, tx) => {
    const metadata = parseMetadata(intent);
    if (!metadata.contestId || !metadata.optionId || !metadata.voterKey) {
        throw new errorHandler_js_1.AppError('Vote payment intent metadata is invalid', 400);
    }
    const voteCount = Math.max(1, Number(metadata.voteCount || 1));
    const existingGrant = await prisma_js_1.default.voteGrant.findFirst({
        where: { paymentIntentId: intent.id },
        select: { id: true },
    });
    if (existingGrant)
    ...
    await prisma_js_1.default.$transaction(async (db) => {
        ...
        if (!option)
            throw new errorHandler_js_1.AppError('Voting option not found', 404);
...
exports.fulfillVotePurchase = fulfillVotePurchase;
//# sourceMappingURL=paymentFulfillment.js.map