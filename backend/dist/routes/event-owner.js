...
const validateOwnerToken = async (req, res, next) => {
...
router.get('/:token', validateOwnerToken, async (req, res) => {
    try {
...
router.get('/:token/rsvps', validateOwnerToken, async (req, res) => {
    try {
        const eventId = req.eventId;
...
router.post('/:token/rsvps/:rsvpId/review', validateOwnerToken, async (req, res) => {
    try {
        const eventId = req.eventId;
...
router.get('/:token/media', validateOwnerToken, async (req, res) => {
    try {
        const eventId = req.eventId;
...
router.get('/:token/checkins', validateOwnerToken, async (req, res) => {
    try {
        const eventId = req.eventId;
...
router.post('/:token/generate-reel', validateOwnerToken, async (req, res) => {
    try {
        const eventId = req.eventId;
...
router.get('/:token/reel/:jobId/status', validateOwnerToken, async (req, res) => {
    try {
...
router.get('/:token/sales', validateOwnerToken, async (req, res) => {
    try {
        const eventId = req.eventId;
...
router.get('/:token/sales/by-ticket', validateOwnerToken, async (req, res) => {
    try {
        const eventId = req.eventId;
...
router.post('/:token/wallet', validateOwnerToken, async (req, res) => {
    try {
        ...
        if (!event?.ownerId) {
            return res.status(400).json({ error: 'Event must be linked to an owner to configure wallet' });
        }
...
router.get('/:token/payouts', validateOwnerToken, async (req, res) => {
    try {
        const eventId = req.eventId;
...
router.post('/:token/payouts/request', validateOwnerToken, async (req, res) => {
    try {
        ...
        // Get owner wallet to check if configured
        if (!event?.ownerId) {
            return res.status(400).json({ error: 'Event must be linked to an owner to request payouts' });
        }
...
router.get('/:token/payouts/:payoutId', validateOwnerToken, async (req, res) => {
    try {
        const eventId = req.eventId;
...
router.delete('/:token/payouts/:payoutId', validateOwnerToken, async (req, res) => {
    try {
        const eventId = req.eventId;
        const { payoutId } = req.params;
        const payout = await prisma.payoutRequest.findFirst({
...
//# sourceMappingURL=event-owner.js.map